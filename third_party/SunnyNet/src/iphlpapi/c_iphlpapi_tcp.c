...
void closeTcpConnectionInit() {
	...
	pGetTcpTable2 = (GETTCPTABLE2_FN)GetProcAddress(hModule, "GetTcpTable2");
...
int getTcpInfoPID(char* Addr, int SunnyProt) {
	...
	for (DWORD i = 0; i < tcpTable->dwNumEntries; i++) {
		sprintf(buf, "%lu.%lu.%lu.%lu:%d",
			(unsigned long)((tcpTable->table[i].dwLocalAddr >> 0) & 0xff),
			(unsigned long)((tcpTable->table[i].dwLocalAddr >> 8) & 0xff),
			(unsigned long)((tcpTable->table[i].dwLocalAddr >> 16) & 0xff),
			(unsigned long)((tcpTable->table[i].dwLocalAddr >> 24) & 0xff),
