...
- MCP 补齐第一版核心能力，支持会话标记颜色、按规则清除会话、请求规则查看、规则启停、规则删除和规则命中记录查询。
- MCP 会话返回字段补充 Host、Query、响应长度、响应类型、进程信息、标记颜色、断点状态、规则命中数量等，方便 AI 直接分析当前抓包状态。
- 完善请求规则中心能力，支持 HTTP/WebSocket/TCP/UDP 屏蔽、请求重写、请求映射等规则的统一查看和管理。
- 优化 WebSocket、TCP、UDP 长连接调试视图，减少无关视图干扰，让消息流和调试区域更清晰。
- 优化会话列表和右侧详情视图性能，大响应、HEX 视图、文本视图切换更流畅。
- 仓库已随项目分发 SunnyNet 核心兼容源码，首次拉取后更方便编译和二次开发。
