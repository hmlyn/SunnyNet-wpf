...
```text
SunnyNet-wpf-v0.1.5-win-x64.zip
```
...
- 程序新增 GitHub Release 版本检查，启动后每日自动检查一次；发现新版本会在主界面轻提示，并可直接打开发布页。
- 程序设置新增“检查更新”按钮，可手动检查当前是否为最新版。
- 进程拦截增强：PID 列表支持输入 PID 或进程名快速搜索，输入 PID 时会联动显示对应进程名。
- 按进程名捕获支持持久化记忆，并可通过勾选控制是否启用捕获。
- 进程名列表和 PID 进程列表支持右键复制进程名。
- 保留 v0.1.4 的查找替换、Base64URL、异步剪贴板复制与 MCP 同步能力。
