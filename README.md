...
```text
SunnyNet-wpf-v0.1.4-win-x64.zip
```
...
- 查找窗口新增查找替换能力，可按 URL、协议头、Body 等范围替换会话内容，替换后的记录也会同步反映到 MCP 读取结果。
- 文本工具新增 Base64URL 编码/解码。
- 加密/解密工具的输入、输出、Key、IV、HMAC Key 均新增 Base64URL 格式支持。
- 优化 HTTP 原文/响应文本和 HEX 视图复制体验，剪贴板被占用时不再阻塞 UI，降低 `Ctrl+C` 导致窗口短暂失去响应的概率。
- 保留 v0.1.3 的 MCP 核心能力、请求规则中心、长连接调试视图和会话列表性能优化。
